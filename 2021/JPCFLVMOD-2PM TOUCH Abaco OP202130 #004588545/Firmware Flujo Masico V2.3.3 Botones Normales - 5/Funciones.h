...
void LimitaValores(void){
   ...
   if(Presion[4].G>300 || Presion[4].G<200){
      Presion[4].G=250;write_eeprom(46,250);write_eeprom(47,0);
   }
   if(Presion[3].G>300 || Presion[3].G<200){
...
void VelocidadMotor(int8 Vel){
   ...
   if(Vel==0){
      ...
      Motor2_off;
      readHolding(); 
      if(make16(txbuf[1],txbuf[2])!=0){//Compara Registro 1 con el valor de activacion de variador
