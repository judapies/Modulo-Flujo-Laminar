...
short Purga=OFF,Cruce=OFF,Cruce2=OFF;
int8 VelMotor=0,VelMotor2=0,Prendio=10,pulsos=0;
...
void ext0() {  
   ...
   if(VelMotor==19){
      ...
      set_timer0(173); // Cada cuenta del timer0 es 51.2us y timer1 es 1.59us
      Motor_off; 
      Motor2_off;
   }else if(VelMotor==7){
      set_timer0(169); // Cada cuenta del timer0 es 51.2us y timer1 es 1.59us
      Motor_off; 
      Motor2_off;
   }else if(VelMotor==6){
      set_timer0(165);      
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==240){ //Menu de Bienvenida.
      ...
      tiempo_purga=0;tiempo_postpurga=0;TipoClave=0;tEncendido=0;VelMotor=0;
      Display_on;
      Encendio=ON;
   }
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==5){ //Menu General.
         ...
         if(imprimir==1){
            //printf("fex.txt=\"Pre-Filtro:%2.2finH20\"",Filtro_Inflow);
            printf("fex.txt=\"Pre-Filtro:----inH20\"");
            SendDataDisplay();
            printf("fdown.txt=\"HEPA:%2.2finH20\"",Filtro_Downflow);
            SendDataDisplay();
            printf("fecha.txt=\"Fecha:%02u/%02u/20%02u    Hora: %02u:%02u:%02u  T:%02u°C\"",Dia,Mes,Year,Hora,Minuto,Segundo,Temperatura);
            SendDataDisplay();
...
// Dibujo de barra de estado de filtro  
      //barraDn=100.0*(((float)Vel1-(float)velocidadInicial)/(20.0-(float)velocidadInicial));
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(Opcion2==1){
         ...
         */
         ...
         if(velocidadInicial>20)velocidadInicial=1;
         if(velocidadInicial<1)velocidadInicial=20;
      }else if(Opcion2==7){
